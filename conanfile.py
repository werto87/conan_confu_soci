...
class ConfuSociConan(ConanFile):
    ...
    def source(self):
        ...
        download("https://github.com/werto87/confu_soci/archive/v0.0.2.zip", zip_name)
        unzip(zip_name)
        os.unlink(zip_name)

    def build(self):
        cmake = CMake(self)
        cmake.configure(source_folder="confu_soci-0.0.2")
        cmake.build()

    def package(self):
        self.copy("*.h*", dst="include/confu_soci",
                  src="confu_soci-0.0.2/convenienceFunctionForSoci")
